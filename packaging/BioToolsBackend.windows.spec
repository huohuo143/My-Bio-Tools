...
ICON = ROOT / "windows" / "MyBioTools.Windows" / "Assets" / "AppIcon.ico"
VERSION_FILE = ROOT / "packaging" / "windows-version-info.txt"

sys.path.insert(0, str(APP_SOURCE))
from tool_catalog import TOOLS_BY_MODULE  # noqa: E402
...
analysis = Analysis(
    ...
    hookspath=[],
    hooksconfig={},
    # Keep python-docx package directories on disk. Version 1.2 resolves
    # header/footer templates through ``docx/parts/../templates`` and the
    # intermediate directory must exist in the frozen runtime.
...
exe = EXE(
    ...
    strip=False,
    upx=False,
    # Keep stdout/stderr available for the WPF shell to redirect into the
    # rotating log. ProcessStartInfo.CreateNoWindow prevents a console window.
    console=True,
