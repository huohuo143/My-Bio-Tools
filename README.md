...
当前 macOS 与 Windows 功能基线均为 **1.9.8（build 27）**。macOS 目标平台为 Apple Silicon（arm64），最低支持 macOS 13；Windows 目标平台为 Windows 10/11 x64，并通过原生 Windows GitHub Actions 构建和验收。
...
- `dist/windows/My-Bio-Tools-1.9.8-build27-win-x64-setup.exe`
- `dist/windows/My-Bio-Tools-1.9.8-build27-win-x64-portable.zip`
...
也可以在 GitHub Actions 中手动运行 `Build and accept Windows 1.9.8 Build 27`。工作流生成安装包和便携版，验证安装、登录界面启动、进程清理和 SHA-256；仅当仓库配置了私有审核账号 secrets 时才执行真实登录与授权多组学运行验收。
