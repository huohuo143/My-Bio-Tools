...
当前 macOS 版本：**1.9.7（build 26）**。macOS 目标平台为 Apple Silicon（arm64），最低支持 macOS 13；Windows 源码仍为 1.9.1，目标平台为 Windows 10/11 x64，本次未构建或发布 Windows 二进制。
...
- `dist/My-Bio-Tools-1.9.7-arm64.dmg`
- `dist/My-Bio-Tools-1.9.7-arm64.dmg.sha256`
- `Resources/AppIcon-1024.png` 与 `Resources/AppIcon.icns`

### Windows

- Windows 源码与版本元数据仍为 v1.9.1；本次不生成或宣称验证 Windows 二进制产物。
