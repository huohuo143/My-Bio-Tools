...
const baseStyles = `
  ...
  main { width:min(720px,calc(100% - 32px)); box-sizing:border-box; background:white; border:1px solid #dce9e6; border-radius:18px; padding:32px; box-shadow:0 18px 50px rgba(20,70,60,.12); }
  ...
  @media (prefers-color-scheme:dark) { body{background:#10201d;color:#edf7f5} main{background:#182d29;border-color:#31514b} input,textarea,select{background:#10201d;color:#edf7f5;border-color:#4d6c66} .muted{color:#a9c1bc} }
`;
...
export function adminDashboard(
  ...
  const rows = users.map((user) => {
    ...
    return `<section style="border-top:1px solid #dce9e6;padding:18px 0">
      ...
      <form method="post" action="/admin/action"><input type="hidden" name="action" value="set_status"><input type="hidden" name="userId" value="${id}"><input name="reason" placeholder="拒绝或停用原因（可留空）"><button name="status" value="active" type="submit">批准/恢复</button> <button name="status" value="rejected" type="submit">拒绝</button> <button name="status" value="suspended" type="submit">停用</button></form>
...
<p class="muted">审核注册、停用账号、强制退出与解绑设备。所有操作都会记录审计日志。</p>
