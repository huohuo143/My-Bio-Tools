...
Windows 版源码与 macOS v1.7.2 共用 `app_source` 中的 Python/Streamlit 工具和内置水稻数据。Windows 原生外壳使用 .NET 10 WPF 与 Microsoft Edge WebView2，最终用户无需安装 Python、Conda 或 .NET。本次发布仅同步 Windows 源码版本，不生成 Windows 二进制安装包。
...
- `My-Bio-Tools-1.7.2-win-x64-setup.exe`
- `My-Bio-Tools-1.7.2-win-x64-portable.zip`
- `SHA256SUMS.txt`
...
构建脚本默认执行源码检查、核心单元测试、五个在线预测适配器的模拟契约测试、全部页面冒烟测试、本地工作流测试和 NLStradamus Windows 二进制自检。最终分发前还应在干净 Windows 10 与 Windows 11 x64 环境逐项验证 7 个工具、Word/Excel/ZIP 下载、WebView2 缺失处理、安装升级与卸载。
